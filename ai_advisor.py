...
{{"parameters":{{"stop_loss":-0.03,"take_profit":0.06,"trailing_activation":0.03,"trailing_distance":0.025,"max_hold_days":25,"min_signals":2,"position_size_pct":0.2}},"reason":"参数调整理由，50字内"}}"""
    ...
    def _build_market_prompt(self, context: dict, news: list) -> str:
        window = context.get("analysis_window") or {}
...
分析口径: {window.get('strategy', '前一交易日完整盘面 + 当日上午盘面')}
执行参考时段: {window.get('execution_window', '13:00-14:00')}

=== 前一交易日完整盘面 ===
沪深300 {previous.get('date', '日期不可用')}：涨跌{float(previous.get('change_pct') or 0):+.2f}%，开{previous.get('open', '--')}，高{previous.get('high', '--')}，低{previous.get('low', '--')}，收{previous.get('close', '--')}

=== 当日上午盘面（截至午间生成时点） ===
数据状态: {morning.get('status', '时间状态待核验')}；截至: {morning.get('as_of', window.get('generated_at', '时间未知'))}
...
"""
        ...
        prompt += """\n请完成A股午间板块轮动研判，供13:00-14:00决策参考：
1. 时间分层：先复盘前一交易日完整K线，再分析当日09:30-11:30上午盘。不得把上午数据写成全天结论；若上午盘未完成，必须降低置信度。
2. 市场状态：结合上涨/下跌家数、涨跌停数量和平均涨跌幅，判断当前是趋势延续、震荡分歧、退潮、超跌反弹还是潜在反转。
3. 每日资金强度：综合板块涨跌幅、主力净流入额、净流入占比和上涨/下跌家数，评估资金强度与板块内部扩散度；不要只按绝对净流入额排序。
4. 外部联动：先总结美股科技、中概股、恒指、原油、黄金、铜及已识别事件的方向，再说明哪些A股板块已获资金确认、哪些只是外部催化候选、哪些出现背离。
5. 轮动路径：指出资金可能流出的板块、正在承接的板块和下一阶段候选板块，并说明外盘、国内政策或新闻事件如何影响该路径。
6. 反弹与反转：单日上涨、单日流入或消息刺激只能判为线索。只有资金持续性、板块扩散度、市场宽度和后续催化共同确认时，才可判为反转；当前数据无法验证连续性时必须标注“待后续交易日确认”。
7. 预测窗口：分别给出当日下午13:00-14:00、未来1-3个交易日和3-10个交易日的条件判断，用高/中/低表示信号强度，并列出触发条件与失效条件。不得把外盘涨跌直接写成A股必然跟涨或跟跌。
8. 标的映射：只从已提供的股票中列出3-5只观察标的，说明其对应板块逻辑；不得编造个股资金或基本面数据。
