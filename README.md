...
推送分析窗口为前一交易日完整盘面和当天09:30–11:30上午盘，供13:00–14:00复核。外盘、美股、商品和地缘事件只形成情景输入，必须由A股板块资金、上涨扩散度和龙头表现确认。邮件始终保留强势板块龙头池和基本面+量化观察池；选股权重初始为基本面40%、量化40%、板块强度10%、上午个股资金强度10%，其中上午资金强度由主力净流入占比换算。每日午间候选会保存时点快照，下一交易日收盘验证后进入受约束权重优化；累计不足20个验证日时保持初始权重。板块、量化模型和上午盘触发继续作为交易许可条件；量化ATR参与止损距离，午后进场还必须由上午盘回踩区或突破条件触发。
