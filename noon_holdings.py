...
def build_noon_action(holding, trend, account, stock, market, sector, now, sector_name=""):
    ...
    for label, quote in contexts.items():
        ...
        if (not quote.get("available") or not session.get("completed") or not valid
    ...
    if issues or not trend.get("available"):
        result.update(action="等待上午行情复核", sell_quantity=0, t_eligible=False, morning_ready=False,
                      reference_price=None, pnl_pct=None,
                      reason="；".join(issues or ["前日趋势数据不足"]))
        return result
