...
class CronEmailEndpointTest(unittest.TestCase):
    ...
    def test_scheduled_push_accepts_weekday_noon_window(self):
        self.assertTrue(server._scheduled_push_allowed(datetime(2026, 8, 17, 11, 50)))
        self.assertTrue(server._scheduled_push_allowed(datetime(2026, 8, 18, 12, 0)))
        self.assertTrue(server._scheduled_push_allowed(datetime(2026, 8, 18, 12, 30)))
        self.assertFalse(server._scheduled_push_allowed(datetime(2026, 8, 17, 8, 0)))
        self.assertFalse(server._scheduled_push_allowed(datetime(2026, 8, 22, 12, 0)))
